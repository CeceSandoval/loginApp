...
interface PopupDriverProps {
  onClose: () => void;
  //sessionId: string | undefined;
}
const PopupDriver: React.FC<PopupDriverProps> = ({ onClose })=> {
  ...
  const fetchUsers = async () => {
      try {
          const response = await axios.get(`http://localhost:8080/driver/${state.session?.id}`)
      ...
      <div className="bg-white rounded-lg p-6">
        ...
        {users ? (
          <div>
            ...
            <p><strong>Placa de vehículo:</strong> {users.licensePlate}</p>
            <p><strong>Modelo de Vehículo:</strong> {users.carModel}</p>
